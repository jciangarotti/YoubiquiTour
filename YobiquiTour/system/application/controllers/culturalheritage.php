...
/**
 * Description of CulturalHeritag
 *
 * @author changa
 */

class Culturalheritage extends Controller{
  ...
  function getRecommendationFromYoubiquitour($userID){
    ...
    foreach($arregloClustersVisitados as &$value){
      ...
      if($booleano){
        $queryConstructor = "SELECT items.itemID as itemID, 
        						items.name as name, 
        						items.positionID as position, 
  ...
  function youbiquitour($clusterID, $user){
    ...
    //Encontrar la lista de los items que están dentro del $cota
